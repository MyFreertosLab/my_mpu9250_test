	...
	while (true) {
		...
		if( ulTaskNotifyTake( pdTRUE,xMaxBlockTime ) == 1) {
			...
			if(counter%100 == 0) {
				...
				printf("Baro: P[%8.8f],T[%8.8f],A[%3.3f], VS[%5.5f]\n", mpu9250_handle->baro.pressure, mpu9250_handle->baro.temperature, mpu9250_handle->baro.altitude, mpu9250_handle->baro.cal.kalman_vspeed.X);
