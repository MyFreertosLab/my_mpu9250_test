...
static esp_err_t mpu9250_gyro_calc_sqm(mpu9250_handle_t mpu9250_handle, int16_t* gyro_means, int16_t* gyro_sqm, uint8_t cycles) {
	ESP_ERROR_CHECK(mpu9250_gyro_calc_var(mpu9250_handle, gyro_means, mpu9250_handle->gyro.cal.var[mpu9250_handle->gyro.fsr].array, 60));
	gyro_sqm[0] = sqrt(mpu9250_handle->gyro.cal.var[mpu9250_handle->gyro.fsr].xyz.x);
	gyro_sqm[1] = sqrt(mpu9250_handle->gyro.cal.var[mpu9250_handle->gyro.fsr].xyz.y);
	gyro_sqm[2] = sqrt(mpu9250_handle->gyro.cal.var[mpu9250_handle->gyro.fsr].xyz.z);
...
static esp_err_t mpu9250_gyro_calc_bias(mpu9250_handle_t mpu9250_handle) {
	// Calc Bias
	{
		...
		ESP_ERROR_CHECK(mpu9250_gyro_calc_sqm(mpu9250_handle, gyro_means, mpu9250_handle->gyro.cal.sqm[mpu9250_handle->gyro.fsr].array, 60));
		printf("Gyro SQM: [%d][%d][%d]\n",
				mpu9250_handle->gyro.cal.sqm[mpu9250_handle->gyro.fsr].array[0],
				mpu9250_handle->gyro.cal.sqm[mpu9250_handle->gyro.fsr].array[1],
				mpu9250_handle->gyro.cal.sqm[mpu9250_handle->gyro.fsr].array[2]);
...
static esp_err_t mpu9250_gyro_calc_offset(mpu9250_handle_t mpu9250_handle) {
	...
	printf("Gyro offsets: [%d][%d][%d]\n", mpu9250_handle->gyro.cal.offset.xyz.x, mpu9250_handle->gyro.cal.offset.xyz.y,mpu9250_handle->gyro.cal.offset.xyz.z);
	...
	while(true) {
		...
		if((found[X_POS] == 0)) {
			if(abs(gyro_means[X_POS]) == 0) {
				found[X_POS] = 1;
				offsets[X_POS] = mpu9250_handle->gyro.cal.offset.xyz.x;
				printf("FOUND X OFFSET\n");
			}
			else {
				mpu9250_handle->gyro.cal.offset.xyz.x -= gyro_means[X_POS];
			}
		} else {
			mpu9250_handle->gyro.cal.offset.xyz.x = offsets[X_POS];
		}
		if((found[Y_POS] == 0)) {
			if(abs(gyro_means[Y_POS]) == 0) {
				found[Y_POS] = 1;
				offsets[Y_POS] = mpu9250_handle->gyro.cal.offset.xyz.y;
				printf("FOUND Y OFFSET\n");
			}
			else {
				mpu9250_handle->gyro.cal.offset.xyz.y -= gyro_means[Y_POS];
			}
		} else {
			mpu9250_handle->gyro.cal.offset.xyz.y = offsets[Y_POS];
		}
		if((found[Z_POS] == 0)) {
			if(abs(gyro_means[Z_POS]) == 0) {
				found[Z_POS] = 1;
				offsets[Z_POS] = mpu9250_handle->gyro.cal.offset.xyz.z;
				printf("FOUND Z OFFSET\n");
			}
			else {
				mpu9250_handle->gyro.cal.offset.xyz.z -= (gyro_means[Z_POS]);
			}
		} else {
			mpu9250_handle->gyro.cal.offset.xyz.z = offsets[Z_POS];
		}
